...
func (msg *RoundOptimizationMessage) ToJSON() ([]byte, error) {
	return json.Marshal(msg)
}

// GetRoutingKey はSQSルーティング用のキーを返す
func (msg *RoundOptimizationMessage) GetRoutingKey() string {
	return fmt.Sprintf("optimization.round.%s", msg.UserID)
}

// GetQueueName はキューナメを返す
func (msg *RoundOptimizationMessage) GetQueueName() string {
	return "round-optimization-queue"
}
...
func (msg *RoundOptimizationMessage) ToLogString() string {
	...
	roundIDShort := msg.RoundID
	if len(roundIDShort) > 8 {
		roundIDShort = roundIDShort[:8] + "..."
	}
	...
	return fmt.Sprintf("RoundOptimization[ID=%s, UserID=%s, RoundID=%s, FocusScore=%d]",
		messageIDShort, userIDShort, roundIDShort, msg.FocusScore)
}