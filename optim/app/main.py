...
def round_optimizer(user_id: uuid.UUID, focus_score: float):
    """ラウンド最適化API
...
def session_optimizer(user_id: uuid.UUID, average_focus_score: float):
    """セッション最適化API
    ...
    # CSVファイルの更新
    new_data = [session_break_time, number_of_round, total_work_time]
    columns = ["session_break_time", "number_of_round", "total_work_time"]
    csv_handler.update_data(new_data=new_data, columns=columns)
