...
def dynamo_round_optimizer(user_id: uuid.UUID, focus_score: float):
    ...
    # DynamoDBにデータを追加
    dynamodb_handler.put_round_data(user_id=str(user_id), time=current_time, focus_score=focus_score)
    print("DynamoDBにデータを追加完了")
    # ラウンド最適化の説明変数と目的変数を取得
...
def dynamo_session_optimizer(user_id: uuid.UUID, avg_focus_score: float):
    ...
    # DynamoDBにデータを追加
    dynamodb_handler.put_session_data(user_id=str(user_id), time=current_time, avg_focus_score=avg_focus_score)
    # セッション最適化の説明変数と目的変数を取得
