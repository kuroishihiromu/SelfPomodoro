...
class CSVHandler:
    ...
    Methods:
        __init__(file_path: str)
         -> CSVファイルの読み込み
        update_data(new_data: list, columns: list[str] = [])
         -> CSVファイルの更新
        choose_data(columns: list[str] = [], rows: list[int] | slice = [])
         -> CSVファイルの特定のカラムと行を取得
        make_chosen_data_list(columns: list[str] = [], rows: list[int] | slice = [])
    ...
    def update_data(self, new_data: list, columns: List[str]):
        ...
        try:
            ...
            # データの追加
            if has_empty_columns and all(col in self.df.columns for col in columns):  # 最後の行に空の列があり、新しいデータがその列に対応する場合
                # 既存の最後の行にデータを追加
